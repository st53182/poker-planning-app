...
app.post('/api/ai-story-analysis', async (req, res) => {
  try {
    const { room_id, story_title, story_description, competence } = req.body;
    ...
    // Map competence to role description
    const competenceDescriptions = {
      'Frontend': 'фронтенд-разработчик, специализирующийся на пользовательском интерфейсе и клиентской части приложения',
      'Backend': 'бэкенд-разработчик, специализирующийся на серверной логике, API и базах данных',
      'Fullstack': 'фулстек-разработчик с опытом работы как на фронтенде, так и на бэкенде',
      'QA': 'тестировщик, специализирующийся на обеспечении качества и тестировании',
      'Analyst': 'аналитик, специализирующийся на анализе требований и бизнес-процессов',
      'Architect': 'архитектор, специализирующийся на проектировании архитектуры системы',
      'Scrum Master': 'Scrum Master, специализирующийся на организации процесса разработки',
      'Project Manager': 'проект-менеджер, специализирующийся на управлении проектом',
      'Team Lead': 'тимлид, специализирующийся на руководстве командой разработки'
    };
    
    const roleDescription = competenceDescriptions[competence] || `специалист с ролью ${competence}`;
    
    const systemPrompt = `Ты - опытный специалист по планированию в Agile команде. Твоя задача - помочь ${roleDescription} проанализировать пользовательскую историю перед оценкой, основываясь на предыдущих оценках в комнате.

Тип оценки: ${estimationType === 'story_points' ? 'Стори поинты (Fibonacci: 0.5, 1, 2, 3, 5, 8, 13, 21, 34)' : 'Часы (1, 2, 4, 8, 16, 24, 32, 40, 48, 56, 64)'}

Роль специалиста: ${competence || 'Не указана'}
...
{
  "risks": ["риск1", "риск2", ...] (массив потенциальных рисков с точки зрения роли специалиста),
  "clarifyingQuestions": ["вопрос1", "вопрос2", ...] (массив уточняющих вопросов для груминга, релевантных для роли),
  "reasoning": "краткое объяснение и анализ истории с точки зрения роли специалиста" (2-3 предложения)
}

Риски должны быть конкретными и полезными, учитывающими специфику роли специалиста. Вопросы должны быть открытыми и помогать уточнить требования, особенно те аспекты, которые важны для роли специалиста.`;

    const userPrompt = `Проанализируй следующую историю с точки зрения ${roleDescription}:
